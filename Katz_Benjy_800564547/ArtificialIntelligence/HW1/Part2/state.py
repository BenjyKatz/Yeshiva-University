...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    for pos in s[0]:
        ...
        required_side_shifts = abs(correct_col-current_col)
        if(correct_col>=current_col):
            required_row_shifts =  (int)(abs(current_pos+required_side_shifts-pos)/n)
        else:
            required_row_shifts =  (int)(abs(current_pos-required_side_shifts-pos)/n)
        current_pos+=1
        total_dist = total_dist + required_row_shifts+required_side_shifts
        # print()
        # print(s[0])
        # print(pos)
        # print("required_side_shifts " + str(required_side_shifts))
        # print("required_row_shifts "+ str(required_row_shifts))
    return total_dist