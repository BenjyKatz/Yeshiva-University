...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value>{
    ...
    public Value get(Key key)
    {
        ...
        if(entry != null && entry.val != null)//the entry.val was added by me
        {
            //System.out.println("The entry is not null the entry val is "+entry.val);
            //we want the entry to be null for values that are written to disk
            return (Value)entry.val;
        }
        else{
            try{
                Value valToReturn = (Value)pm.deserialize(key);
                put(key, valToReturn);
                return valToReturn;
            }
            catch(Exception e){
