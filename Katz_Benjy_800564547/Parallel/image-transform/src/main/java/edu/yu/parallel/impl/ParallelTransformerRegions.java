...
public class ParallelTransformerRegions implements ImageTransformer {

    private BufferedImage originalImage;

    public ParallelTransformerRegions(BufferedImage originalImage) {
    ...
    */
    ...
    private void resizeImageParallel(BufferedImage resizedImage, double scaleFactor) {
        ...
        try {
            int blockHeight = newHeight / numThreads;

            for (int i = 0; i < numThreads; i++) {
                final int startY = i * blockHeight;
                final int endY = (i == numThreads - 1) ? newHeight : (i + 1) * blockHeight;
                ...
                futures.add(executor.submit(() -> {
                    ...
                    try {
                        ...
                        // Draw the image for the current block, scaling it to fit the new size
                        graphics.drawImage(originalImage,
                                0, startY, newWidth, endY,
                                0, (int) (startY / scaleFactor), originalImage.getWidth(), (int) (endY / scaleFactor),
                                null);
                    } finally {
                        graphics.dispose();
                    }
